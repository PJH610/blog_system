...
public class UserContorller extends BaseApiController{
    @Autowired
    ...
    public Map<String, Object> alterPw(@RequestParam String password, @RequestParam String new_password, @RequestParam String re_password,  HttpServletRequest request) {
        ...
        if (new_password.equals(re_password)) {
            ...
            if (user == null || !user.getPassword().equals(Md5Utils.encrypt(user.getMobilePhone(), password.trim())))
                return onBadResp("密码修改失败");

            User updateUser = new User();
            updateUser.setId(user.getId());
            updateUser.setPassword(Md5Utils.encrypt(user.getMobilePhone(), new_password.trim()));
